...
[Top](#36-valid-sudoku) | [Problem](#problem)
...
var isValidSudoku = function(board) {
    ...
    for(let row = 0; row < 9; row++) {
        for(let col = 0; col < 9; col++) {
            ...
            if(v >= '0') {
                const squareIdx = 3 * Math.floor(row / 3) + Math.floor(col / 3);
                if(rows[row].has(v) || cols[col].has(v) || squares[squareIdx].has(v)) {
                    return false;
                }
                rows[row].add(v);
                cols[col].add(v);
                squares[squareIdx].add(v);
            }
        }
    }
